...
public class EnvironmentController {
    ...
    public @ResponseBody ResponseEntity<?> getWorkspaces(@PathVariable Long environmentId) {
        ...
        GeoServerClient client = Feign.builder()
            .decoder(new GsonDecoder())
        	.requestInterceptor(new BasicAuthRequestInterceptor(environment.getGeoserverUser(), environment.getGeoserverPassword()))
        	.target(GeoServerClient.class, environment.getGeoserverUrl());
        
        List<GeoServerWorkspace> gsWorkspaces = client.workspaces().getWorkspaces().getWorkspace();
        // Go through each GeoServer workspace and check if it is in the environment's list.
        List<Resource<Workspace>> resourceList = gsWorkspaces.stream().map(gsWorkspace -> {
            Workspace workspace;
            // Find out if this particular GeoServer work space is in the environment's list.
            Optional<Workspace> optionalWorkspace = environment.getWorkspaces().stream()
                    .filter(ws -> {
                        return gsWorkspace.getName().equals(ws.getName());
                    })
                    .findFirst();
            if (optionalWorkspace.isPresent()) {
                // If it is in the list, set the transient registered flag to true.
                workspace = optionalWorkspace.get();
                workspace.setRegistered(true);
            } else {
                // If it is not in the list, create a new Workspace object with the name from GeoServer,
            ...
            // Create a Resource for each Workspace and set the appropriate links.
